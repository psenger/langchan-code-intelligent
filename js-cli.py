...
import torch  # Required for the model to work
...
class CodeBERTEmbeddings:
    ...
    def encode(self, texts):  # Add this method
...
class VectorDatabase:
    ...
    def _chunk_code(self, content: str, max_tokens: int = 200) -> List[str]:  # Reduced max_tokens
        """Split code into chunks based on token count"""

        def check_token_count(text):
            return len(self.embeddings_model.tokenizer.encode(text))

        def is_valid_chunk(text):
            tokens = check_token_count(text)
            return tokens < 500  # Buffer below the 512 limit

        def force_split_chunk(text, target_size=200):
            words = text.split()
            chunks = []
            current_chunk = []
            ...
            for word in words:
                ...
                if check_token_count(chunk_text) > target_size:
                    if len(current_chunk) > 1:
                        # Save everything except the last word
        ...
        for line in lines:
            ...
            # If adding this line would make the chunk too big
            if current_chunk and not is_valid_chunk('\n'.join(current_chunk + [line])):
                # Save current chunk
                chunk_text = '\n'.join(current_chunk)
            ...
            # Split on code boundaries regardless of size
            if line.startswith(('function', 'class', 'const', 'let', 'var', 'import', 'export')):
                if current_chunk:
                    chunk_text = '\n'.join(current_chunk)
                    if is_valid_chunk(chunk_text):
                        chunks.extend(force_split_chunk(chunk_text))
        ...
        for chunk in chunks:
            if chunk and is_valid_chunk(chunk):
                ...
                tokens = check_token_count(chunk)
                if tokens > 500:
                    # One final attempt to split
                    sub_chunks = force_split_chunk(chunk, target_size=150)  # Even smaller chunks
                    for sub in sub_chunks:
                        if is_valid_chunk(sub):
                            verified_chunks.append(sub)
    ...
    def _process_batch(self, documents: List[CodeDocument], table):
        ...
        for doc in documents:
            # Split by function/class definitions or use a simpler approach like line count
        ...
        embeddings = self.embeddings_model.encode(chunked_texts)
        ...
        data = [
            ...
            for metadata, vector in zip(chunk_metadata, embeddings)
        ]
...
class ChainFactory:
    ...
    def _create_refactor_chain(self):
        template = """You are an expert JavaScript/TypeScript refactoring assistant. Analyze the following code and suggest improvements.
        ...
        1. Modern JS/TS Patterns
            - ES6+ features and idioms
        ...
        5. Type System Improvements (if TypeScript)
        """
        return self._create_chain(template)

    def _create_feature_chain(self):
        template = """You are an expert JavaScript/TypeScript developer. Suggest implementation for a new feature.
    ...
    def _create_bug_chain(self):
        template = """You are an expert JavaScript/TypeScript debugger. Analyze the following code for potential bugs and issues.
        ...
        4. State Management
            - State mutations
            - Props/state synchronization
            - Side effects
            - Context usage
            - Store management (Redux/MobX)
    ...
    def _create_custom_chain(self):
        template = """You are an expert JavaScript/TypeScript code assistant. Use the following code context to answer the question.
        ...
        Include complete, working code snippets with appropriate types and documentation.
        """
        return self._create_chain(template)
